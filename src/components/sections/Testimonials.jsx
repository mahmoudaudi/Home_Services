...
function NavButton({ direction, onClick, disabled = false, className = "" }) {
  ...
  const colorClass = disabled
    ...
    : "bg-[#00A3E0] hover:bg-[#0089BE] text-white shadow-sm active:scale-95";

  return (
    <button
      onClick={disabled ? undefined : onClick}
      disabled={disabled}
      className={`rounded-full flex items-center justify-center group transition-colors ${colorClass} ${className}`}
      aria-label={isNext ? "Next review" : "Previous review"}
    >
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2.5" stroke="currentColor" className={`${disabled ? "" : translateClass} transition-transform`} aria-hidden="true">
  ...
  return (
    <section className="bg-white py-12 px-6 md:px-12 lg:py-20 lg:px-20 select-none w-full flex flex-col items-center justify-center overflow-hidden">
      <div className="max-w-[1050px] w-full mx-auto flex flex-col items-center text-center">
        ...
        <h2 className="text-[26px] sm:text-[30px] md:text-[36px] lg:text-[46px] font-bold text-[#1E293B] leading-tight lg:leading-[52px] tracking-tight max-w-[700px] mb-6 lg:mb-8">
        ...
        <div className="relative w-full flex items-center justify-between gap-4 md:gap-8 min-h-[140px] mb-6">
          <NavButton direction="prev" onClick={handlePrev} disabled={isFirst} className="hidden lg:flex shrink-0 w-11 h-11 [&>svg]:w-4 [&>svg]:h-4" />
          <div key={activeIndex} className="testimonial-fade max-w-[680px] mx-auto px-2" aria-live="polite" aria-atomic="true">
            <p className="text-slate-500/90 text-[14.5px] md:text-[15.5px] leading-[24px] md:leading-[26px] font-normal text-center">
              {active.text}
            </p>
          </div>
          <NavButton direction="next" onClick={handleNext} disabled={isLast} className="hidden lg:flex shrink-0 w-11 h-11 [&>svg]:w-4 [&>svg]:h-4" />
        </div>

        <div key={`author-${activeIndex}`} className="testimonial-fade flex items-center gap-3.5 text-left justify-center mb-8 lg:mb-10">
          <div className="w-14 h-14 rounded-full overflow-hidden border border-slate-100 shadow-sm shrink-0">
            <img src={AVATARS[activeIndex]} alt={active.author} className="w-full h-full object-cover" />
          </div>
        ...
        <div className="flex lg:hidden items-center justify-center gap-6 w-full">
          <NavButton direction="prev" onClick={handlePrev} disabled={isFirst} className="w-[50px] h-[50px] [&>svg]:w-5 [&>svg]:h-5" />
          <NavButton direction="next" onClick={handleNext} disabled={isLast}  className="w-[50px] h-[50px] [&>svg]:w-5 [&>svg]:h-5" />
        </div>
